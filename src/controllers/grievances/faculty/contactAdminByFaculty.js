...
export const getContactAdminByFaculty = async (req, res) => {
  const { facultyId } = req.params;
  try {
    const query =
      "SELECT * FROM grievance WHERE reciever=? AND sender=? AND faculty=?";
    const [rows] = await db.query(query, ["admin", "faculty", facultyId]);
...
export const createContactAdminByFaculty = async (req, res) => {
...
export const updateContactAdminByFaculty = async (req, res) => {
...
export const deleteContactAdminByFaculty = async (req, res) => {
  const { gid } = req.params;
  try {
    const query = `
...
export const getContactAdminByFacultyByGid = async (req, res) => {
  const { gid } = req.params;
  try {
    const query = `
